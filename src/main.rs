...
// Fix for redb 4.1: WriteStrategy is no longer at the root; use Database directly
use redb::Database; 
...
async fn run_app() -> Result<(), Box<dyn std::error::Error>> {
    let cfg = config::load_config(); // [cite: 83]
    ...
    // 1) Initialize redb 4.1 Database
    ...
    // Spawn Tasks with proper Arc cloning
